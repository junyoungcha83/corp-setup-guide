...
function renderRef(){
  ...
  const catBox=document.getElementById('refCats');
  const editable=!!getEditToken();
  const catOf = v => (VIDEO_CATS.includes(v.cat) ? v.cat : '기타');

  // 중분류 필터 탭(칩) — 전체 + 영상 있는 분류(개수)
  const present = VIDEO_CATS.filter(c => state.videos.some(v => catOf(v)===c));
  if (!present.includes(refCat) && refCat!=='전체') refCat='전체';
  const chips = [['전체', state.videos.length], ...present.map(c=>[c, state.videos.filter(v=>catOf(v)===c).length])];
  catBox.innerHTML = state.videos.length
    ? chips.map(([c,n])=>`<button class="ref-cat${c===refCat?' active':''}" data-cat="${escapeAttr(c)}">${escapeHtml(c)} <b>${n}</b></button>`).join('')
    : '';
  catBox.querySelectorAll('.ref-cat').forEach(b=>b.onclick=()=>{ refCat=b.dataset.cat; renderRef(); });
  ...
  const cardHtml = v => {
    ...
    const actions = editable
      ? `<div class="v-actions"><button class="v-edit" data-edit="${escapeAttr(v.id)}">✏️</button><button class="v-del" data-del="${escapeAttr(v.id)}">🗑</button></div>` : '';
    return `<div class="v-card" data-id="${escapeAttr(v.id)}"><div class="v-titlebar">${title}</div>${media}${actions}</div>`;
  };
  const list = state.videos
    .filter(v => refCat==='전체' || catOf(v)===refCat)
